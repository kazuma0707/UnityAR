...
public class ARCenterButton : MonoBehaviour
{
    ...
    {
        //  ジャンプ不可能な状態なら
        if (script.GetComponent<ARUnityChanControlScriptWithRgidBody>().GetJumpPossibleFlag() == false)
        {
            //  ボタンの透明度を変える
            ButtonAlphaChange();
        }
    ...
    {
        script.GetComponent<ARUnityChanControlScriptWithRgidBody>().CenterButtonFlag = true;
    }
